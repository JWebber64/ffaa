...
Manager-owned team identity marks use `src/ui/TeamIdentityMark.tsx`. The Sleeper user image comes from `managerAvatarUrl`; `avatarUrl` remains league artwork and must not be substituted for manager identity. The workspace header, My Team summary, and My Teams rows all render the shared image with their existing initials as the missing-image or load-error fallback.
