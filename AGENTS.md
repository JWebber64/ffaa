...
- `PositionBadge` owns the exact opaque semantic fill. Feature modifier classes may change size, spacing, radius, border, and typography, but they must not tint, alpha-mix, surface-mix, or replace its background.
...
- Before a Production release, verify rendered QB, RB, WR, TE, FLEX, K, DEF/DST, and bench examples on every changed badge surface, including Build a Team, Auction Values, player lists, roster configuration, and draft team details. Inspect the computed `background-color`, not only the class or source token. For numbered roster slots, verify that `RB1` resolves to the exact RB green fill and `WR1` resolves to the exact WR blue fill.
