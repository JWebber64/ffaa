...
Result lead closings are allocated only after all paired matchups for a week are known. `allocateWeeklyLeadClosings` uses deterministic, margin-aware variants plus a per-week used-text set, so the same closing sentence cannot repeat within the weekly edition while prepared variants remain. New result copy must be added to a variant pool and covered by the weekly uniqueness test; do not add a single generic fallback inside `buildMatchupRecap`.
...
- Reuse `generateWeeklyAwards` (`weekly-awards-v3`), retaining read compatibility with v1 and v2 persisted records. No award/import writes are initiated by reading an edition.
...
- Natural-position awards consider only players whose recorded starting slot matches that natural position; a TE/RB/WR started in FLEX cannot win the TE/RB/WR honor. FLEX/SFLEX/restricted-flex honors require the actual recorded slot; they are separate honors, not extra points. Unknown slots suppress slot honors. Each player honor uses the player's one recorded starting slot.
