...
export function metadataForPath(pathname: string): RouteMetadataInput {
  ...
  if (/^\/league\/[^/]+\/commissioner\/draft(?:\/|$)/.test(pathname)) return { title: "Commissioner Draft", description: "Configure, launch, pause, recover, and audit the native league draft.", path: pathname, indexable: false };
  ...
  if (/^\/league\/[^/]+\/draft(?:\/|$)/.test(pathname)) return { title: "Native League Draft", description: "Reconnect to the authoritative GameHQ draft room, queue players, and make team selections.", path: pathname, indexable: false };
