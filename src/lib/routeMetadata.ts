...
export function metadataForPath(pathname: string): RouteMetadataInput {
  ...
  if (/^\/league\/[^/]+\/(?:standings|teams|matchups|transactions|manage)(?:\/|$)/.test(pathname)) return { title: "League Workspace", description: "Review teams, standings, matchups, transactions, and settings for the active fantasy league.", path: pathname, indexable: false };
