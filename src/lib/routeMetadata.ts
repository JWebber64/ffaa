...
export function metadataForPath(pathname: string): RouteMetadataInput {
  ...
  if (pathname === "/league") return { title: "League HQ", description: "Connect Sleeper leagues, import completed seasons, and manage the settings behind GameHQ league tools.", path: pathname };
  ...
  if (pathname.startsWith("/league/matchups")) return { title: "Fantasy Matchups", description: "Compare weekly fantasy matchups using saved manager lineups and transparent projection baselines.", path: pathname, indexable: false };
  if (pathname.startsWith("/league/lineup")) return { title: "Set Weekly Lineup", description: "Choose legal weekly starters and save a manager lineup for the active fantasy league.", path: pathname, indexable: false };
