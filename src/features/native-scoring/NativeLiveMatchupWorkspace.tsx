...
import { nativeScoreStatus } from "./nativeScoreStatus";
...
export function NativeLiveMatchupWorkspace({ workspace, personalOnly }: { workspace: CanonicalLeagueWorkspace; personalOnly: boolean }) {
  ...
  return <div className="native-scoring-workspace">
    ...
    {selected ? <MatchupBoard matchup={selected} teamById={teamById} lineupByTeam={lineupByTeam} scoringWeek={state.scoringWeek!} directory={directory} openPlayer={(playerId) => openPlayer({ playerId, currentWeek: week, leagueState: "owned", ownership: "Matchup roster", rosterFit: "Current matchup contributor", actionLabel: "Open team lineup", actionTo: `/league/${workspace.league.id}/team?week=${week}` })} /> : <section className="native-scoring-empty"><Trophy aria-hidden="true" /><h2>No Week {week} scoring slate</h2><p>A commissioner can publish the team pairings below. Until then, GameHQ will not present projection baselines as live scores.</p></section>}
...
function MatchupBoard({ matchup, teamById, lineupByTeam, scoringWeek, directory, openPlayer }: { matchup: NativeScoringMatchup; teamById: Map<string, { name: string }>; lineupByTeam: Map<string, { benchPoints: number; optimalScore: number; optimalDelta: number }>; scoringWeek: NonNullable<ReturnType<typeof useNativeScoring>["scoringWeek"]>; directory: Map<string, { name: string }>; openPlayer: (playerId: string) => void }) {
