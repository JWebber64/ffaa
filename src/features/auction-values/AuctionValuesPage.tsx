    ...
    <div className={`auction-values-page ${isPrintRoute ? "is-print-route" : ""} ${state.inkFriendly ? "is-ink-friendly" : ""}`}>
      ...
      <header className="auction-page-header">
        <div>
          ...
          <p>Compare public salary-cap values, market prices, and cheat sheets across Standard, Half PPR, and Full PPR formats.</p>
        </div>
      ...
      <div className="auction-definition-strip" aria-label="Auction value definitions">
        ...
        <div><strong>Consensus</strong><span>Aggregate of compatible selected sources; expert-only by default.</span></div>
        <div><strong>League Price</strong><span>Reserved for future league-history calculations.</span></div>
      </div>
      ...
      {PLAYER_MATCH_WARNINGS.length ? (
        <details className="auction-match-warnings">
          ...
          <p>These rows are retained with explicit unmatched IDs and are never silently merged with a similarly named player.</p>
