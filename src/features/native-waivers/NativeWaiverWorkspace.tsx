      ...
      {!state.waiverState ? (
        ...
        <>
          ...
          <div className="native-waiver-grid">
            <section className="native-waiver-market" aria-labelledby="native-market-title"><header><div><span>Availability</span><h2 id="native-market-title">Player market</h2></div><small>{available.length} claimable</small></header><div className="native-waiver-player-head" aria-hidden="true"><span>Player</span><span>Status</span><span>Eligible</span></div>{available.slice(0, 60).map((entry) => { const player = directory.get(entry.playerId); return <div className="native-waiver-player" key={entry.playerId}><div><PositionBadge position={entry.position} /><span><strong>{player?.name ?? entry.playerId}</strong><small>{player?.team || "FA"} · Rank {player?.rank ?? "—"}</small></span></div><strong>{entry.state === "on_waivers" ? "Waivers" : "Free agent"}</strong><small>{entry.droppedUntil ? time(entry.droppedUntil, workspace.league.timezone) : "Now"}</small></div>; })}</section>
