    ...
    <ToolLayout eyebrow="Auction room" title="Build a Team" description="Set your wallet and roster demand, then draft a team against a sortable fair-value and market board." methodology={<p>Fair values are recalculated directly for your budget, league size, scoring, and roster depth. Market is the median of compatible imported auction-dollar sources. Your actual bid is the number you enter; the recommendation is a starting point, not a prediction.</p>}>
      ...
      <div className="auction-builder-grid">
        <section className="auction-team-card" aria-labelledby="auction-team-title">
          ...
          <div className="auction-card-slots">{SLOT_ORDER.map((slot) => { const slotPicks = cardGroups[slot]; return <div className="auction-card-group" key={slot}><div className="auction-group-label"><span>{SLOT_LABELS[slot]}</span><small>{slot === "FLEX" ? "RB / WR / TE" : `${slotPicks.length}/${slots[slot]}`}</small></div>{slotPicks.map(({ player, bid: playerBid }) => <div className="auction-drafted-player" key={player.id}><span className={`tool-position-tag is-${player.position.toLowerCase()}`}>{player.position}</span><div><strong>{player.name}</strong><small>{player.team} · {player.projectedPoints?.toFixed(1) ?? "—"} pts</small></div><b>{money(playerBid)}</b><button type="button" aria-label={`Remove ${player.name}`} onClick={() => setPicks((current) => current.filter((pick) => pick.playerId !== player.id))}><Trash2 size={14} /></button></div>)}{!slotPicks.length && slot !== "BENCH" ? <div className="auction-open-slot">Open slot</div> : null}</div>; })}</div>
        </section>
